    ...
    handler: async (
        ...
        try {
            ...
            if (transferOptions.callBackTemplate) {
                elizaLogger.debug(traceId, `[transferTokenAction] [${moxieUserId}] [processMessage] transferOptions: ${JSON.stringify(transferOptions)}`);
            ...
            if (!validationResult) {
                elizaLogger.debug(traceId, `[transferTokenAction] [${moxieUserId}] [isValidTransferContent] validationResult: ${JSON.stringify(validationResult)}`);
            ...
            elizaLogger.error(traceId, `[tokenTransferAction] [${moxieUserId}] [ERROR] error: ${error}`);
 ...
 */
function handleValidations(moxieUserId: string, runtime: IAgentRuntime, message: Memory, state: State) {
...
async function isValidTransferContent(
    ...
    elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] content: ${JSON.stringify(content)}`);

    // Validate basic content structure
    if (!content?.transfers?.length) {
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] Invalid content structure: ${JSON.stringify(content)}`);
        return {
    ...
    for (const transfer of content.transfers) {
        ...
        if (!transfer.sender || !transfer.recipient || !transfer.token) {
            elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] Missing required fields in transfer: ${JSON.stringify(transfer)}`);
            return {
        ...
        if (!transfer.balance && (!transfer.transferAmount || transfer.transferAmount <= 0)) {
            elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] Invalid quantity: transferAmount=${transfer.transferAmount}`);
            return {
        ...
        if (transfer.value_type && transfer.value_type !== 'USD') {
            elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] Invalid value_type: ${transfer.value_type}`);
            return {
        ...
        if (transfer.balance && transfer.balance.type) {
            ...
            if (!source_token || !type || !['FULL', 'PERCENTAGE'].includes(type)) {
                elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] Invalid balance configuration: ${JSON.stringify(transfer.balance)}`);
                return {
            ...
            if (type === 'PERCENTAGE' && (typeof percentage !== 'number' || percentage <= 0 || percentage > 100)) {
                elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [isValidTransferContent] Invalid percentage in balance: ${percentage}`);
                return {
...
async function processMessage(
    ...
    elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] transferOptions: ${JSON.stringify(transferOptions)}`);
...
async function preValidateRequiredData(context: Context) {
    ...
    if (!state.moxieWalletClient) {
        elizaLogger.error(context.traceId, `[tokenTransfer] [preValidateRequiredData] Moxie wallet client not found`);
        return callBackTemplate.APPLICATION_ERROR("Moxie wallet client not found in state");
    }
...
async function processTransfer(
    ...
    for (const transfer of transferOptions.transfers) {
        elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processTransfer] Processing transfer: ${JSON.stringify(transfer)}`);
...
async function processSingleTransfer(
    ...
    try {
        ...
        if (!tokenAddressResult.data) {
            elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processTransfer] resolvedTokenAddress not found`);
            return {
                callBackTemplate: tokenAddressResult.callBackTemplate
            };
        }
        elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processTransfer] resolvedTokenAddress: ${JSON.stringify(tokenAddressResult.data)}`);
        ...
        elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processTransfer] transferResult: ${JSON.stringify(transferResult)}`);
        ...
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processSingleTransfer] Error: ${error}`);
        return {
            callBackTemplate: callBackTemplate.APPLICATION_ERROR(`Error processing transfer: ${error.message}`)
        };
    }
...
async function resolveRecipientAddress(context: Context, recipient: string): Promise<FunctionResponse<string>> {
    if (!recipient) {
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] No recipient provided`);
        return {
            callBackTemplate: callBackTemplate.APPLICATION_ERROR("Recipient is required")
        };
    }

    elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] Resolving recipient: ${recipient}`);

    try {
        // First check if it's a valid Ethereum address since that's fastest
        if (ethers.isAddress(recipient)) {
            elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] Valid Ethereum address: ${recipient}`);
            return {
                data: recipient
            };
        }
        ...
        if (ensResult.resolvedAddress) {
            elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] Resolved ENS address: ${ensResult.resolvedAddress}`);
            return {
        ...
        if (creatorCoinResult?.data) {
            elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] Resolved creator coin: ${creatorCoinResult.data}`);
            return {
                data: creatorCoinResult.data
            };
        }

        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] Unable to resolve recipient: ${recipient}`);
        return {
            callBackTemplate: callBackTemplate.APPLICATION_ERROR(`Unable to resolve recipient address`)
        };

    } catch (error) {
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveRecipientAddress] Error resolving recipient: ${error}`);
        return {
...
async function processCreatorCoin(context: Context, recipient: string): Promise<FunctionResponse<string>> {
    try {
        ...
        elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processCreatorCoin] recipientTokenCreatorId: ${recipientTokenCreatorId} and recipientTokenCreatorUsername: ${recipientTokenCreatorUsername}`);

        if (!recipientTokenCreatorId) {
            elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processCreatorCoin] recipientTokenCreatorId not found`);
            return null;
        }
        ...
        if (!moxieUserDetails) {
            elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processCreatorCoin] moxieUserDetails not found`);
            return null;
        }
        const creatorAgentWallet = moxieUserDetails.wallets.filter((Wallet) => Wallet.walletType === "embedded")[0];

        if (!creatorAgentWallet) {
            elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processCreatorCoin] creatorAgentWallet not found`);
            return null;
        }
        ...
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processCreatorCoin] Error: ${error}`);
        return {
...
async function processERC20(context: Context, recipient: string): Promise<FunctionResponse<string>> {
    try {
        ...
        elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processERC20] tokenSymbol: ${tokenSymbol} and tokenAddress: ${tokenAddress}`);

        // fetch the token details
        if (!tokenSymbol || !tokenAddress) {
            elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processERC20] Invalid token details: ${recipient}`);
            return {
        ...
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [processERC20] Error: ${error}`);
        return {
...
async function getFtaResponses(
    ...
    if (!creatorIds?.length) {
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [getFtaResponses] No creator IDs provided`);
        return {
    ...
    const results = await Promise.all(creatorIds.map(async (creatorId) => {
        ...
        try {
            ...
            if (cachedResponse) {
                elizaLogger.debug(
                    context.traceId, `[tokenTransfer] [${context.moxieUserId}] [getFtaResponses] Cache hit for creator ${creatorId}`
                );
            ...
            elizaLogger.debug(
                context.traceId, `[tokenTransfer] [${context.moxieUserId}] [getFtaResponses] Cached new data for creator ${creatorId}`
            );
            return { creatorId, response: newResponse };

        } catch (error) {
            elizaLogger.error(
                context.traceId, `[tokenTransfer] [${context.moxieUserId}] [getFtaResponses] Error processing creator ${creatorId}: ${error}`
            );
            return { error: true, creatorId };
        }
    }));
...
async function resolveTokenAddress(context: Context, token: string): Promise<FunctionResponse<TokenDetails>> {
    elizaLogger.debug(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveTokenAddress] Resolving token: ${token}`);
    try {
        ...
        if (subjectTokenDetails.data) {
            return {
                data: {
                    ...
                    currentMoxiePriceInWEI: subjectTokenDetails.data.currentPriceInWeiInMoxie
                }
            };
        }
        ...
        elizaLogger.error(context.traceId, `[tokenTransfer] [${context.moxieUserId}] [resolveTokenAddress] Error: ${error}`);
        return {
...
async function executeTransfer(
    ...
    elizaLogger.debug(
        context.traceId, `[tokenTransfer] [${context.moxieUserId}] [executeTransfer] Executing transfer of ${amountInWEI} tokens from ${agentWallet} to ${recipientAddress}`
    );

    const feeData = await context.provider.getFeeData();
    elizaLogger.debug(
        context.traceId, `[tokenTransfer] [${context.moxieUserId}] [executeTransfer] feeData: ${JSON.stringify(feeData)}`
    );
    ...
    elizaLogger.debug(
        context.traceId, `[tokenTransfer] [${context.moxieUserId}] [executeTransfer] maxPriorityFeePerGas: ${maxPriorityFeePerGas} maxFeePerGas: ${maxFeePerGas}`
    );
    ...
    elizaLogger.debug(
        context.traceId, `[tokenTransfer] [${context.moxieUserId}] [executeTransfer] request: ${JSON.stringify(request)}`
    );
    ...
    elizaLogger.debug(
        context.traceId, `[tokenTransfer] [${context.moxieUserId}] [executeTransfer] transactionResponse: ${JSON.stringify(transactionResponse)}`
    );
