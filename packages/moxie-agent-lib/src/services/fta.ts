...
export async function getFtaUserMapping(entitySymbols: string[]): Promise<FtaUserMapping[]> {
    const query = `
    ...
    try {
        const response = await fetch(process.env.AIRSTACK_BACKEND_GRAPHQL_ENDPOINT, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
