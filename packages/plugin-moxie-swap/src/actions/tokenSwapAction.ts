...
export const tokenSwapAction = {
    ...
    handler: async (
        ...
        try {
            ...
            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] swapOptions: ${JSON.stringify(swapOptions)}`);

            // check if confirmation is required
            if (swapOptions.confirmation_required) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] confirmation required: ${swapOptions.confirmation_required}`);
                await callback?.({
            ...
            if (swapOptions.error) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] swapOptions has error: ${JSON.stringify(swapOptions)}`);
                await callback?.({
            ...
            if (!isValidSwapContent(traceId, moxieUserId, swapOptions, callback)) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] swapOptions is not valid: ${JSON.stringify(swapOptions)}`);
                return true;
            }
            ...
            if (!agentWallet) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] agentWallet not found`);
                await callback?.(agentWalletNotFound);
                return true;
            }

            if (!agentWallet.delegated) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] agentWallet is not delegated`);
            ...
            if (!walletClient) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] walletClient not found`);
            ...
            for (const transaction of swapOptions.transactions) {
                ...
                if (swapOptions.action == "SWAP" || swapOptions.action == "SELL" || swapOptions.action == "BUY") {
                    ...
                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [SUBJECT_TOKEN_CHECK] subjectTokenDetails: ${JSON.stringify(subjectTokenDetails)}`);
                    ...
                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [SUBJECT_TOKEN_CHECK] isSellTokenCreatorCoin: ${isSellTokenCreatorCoin}`);
                    ...
                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [SUBJECT_TOKEN_CHECK] isBuyTokenCreatorCoin: ${isBuyTokenCreatorCoin}`);
                    ...
                    const swapType = (hasSellCreator && hasBuyCreator) ? 'CREATOR_TO_CREATOR'
                        : (hasSellCreator) ? 'CREATOR_TO_TOKEN'
                            : (hasBuyCreator) ? 'TOKEN_TO_CREATOR'
                                : 'TOKEN_TO_TOKEN';
                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [SWAP_TYPE] swapType: ${swapType}`);


                    if (swapType == 'CREATOR_TO_CREATOR') {
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] starting creator to creator swap`);
                        ...
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] sellTokenSubjectTokenAddress: ${sellTokenSubjectTokenAddress} and buyTokenSubjectTokenAddress: ${buyTokenSubjectTokenAddress}`);
                        ...
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] buyTokenDecimals: ${buyTokenDecimals} and sellTokenDecimals: ${sellTokenDecimals}`);
                        ...
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] sellTokenSubjectAddress: ${sellTokenSubjectAddress} and buyTokenSubjectAddress: ${buyTokenSubjectAddress}`);
                        ...
                        if (buyQuantity) {
                            ...
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] updatedBuyQuantity: ${updatedBuyQuantity}`);
                            // Calculate moxie required for buy quantity
                            const buyTokenCurrentPriceInWEI = Decimal(buyTokenSubjectTokenDetails.currentPriceInWeiInMoxie);
                            if (value_type && value_type == "USD") {  // usd based operations
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BUY_QUANTITY] [VALUE_TYPE]: ${usdQuantityInWEI}`);
                                try {
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] [BUY_QUANTITY] price: ${price}`);
                                    const buyAmountInWEI = BigInt(price);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] [BUY_QUANTITY] buyAmountInWEI: ${buyAmountInWEI}`);

                                    // check how many tokens can be bought with the moxie
                                    const requiredBuyQuantityInWEI = await calculateTokensBuy(traceId, moxieUserId, buyTokenSubjectAddress, buyAmountInWEI);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] [BUY_QUANTITY] requiredBuyQuantityInWEI: ${requiredBuyQuantityInWEI}`);

                                    // convert wei into format units
                                    const requiredBuyQuantity = ethers.formatUnits(requiredBuyQuantityInWEI, buyTokenDecimals);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] [BUY_QUANTITY] requiredBuyQuantity: ${requiredBuyQuantity}`);

                                    updatedBuyQuantity = Number(requiredBuyQuantity);
                                } catch(error) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] [BUY_QUANTITY] [ERROR] Error: ${error}`);
                                    return true;
                                }
                            }
                            const moxieRequiredInWEI = Decimal(updatedBuyQuantity.toString())
                            ...
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] moxieRequiredInWEI: ${moxieRequiredInWEI}`);

                            // now check the selling token
                            const sellTokenCurrentPriceInWEI = Decimal(sellTokenSubjectTokenDetails.currentPriceInWeiInMoxie);
                            const requiredSellQuantity = Decimal(moxieRequiredInWEI)
                                .div(sellTokenCurrentPriceInWEI)
                                .toFixed(sellTokenDecimals)  // Force exactly 18 decimal places
                                .replace(/\.?0+$/, ''); // Remove trailing zeros and decimal point if whole number
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [REQUIRED_SELL_QUANTITY] requiredSellQuantity: ${requiredSellQuantity}`);
                            ...
                            if (BigInt(currentSellTokenBalanceInWEI) < requiredSellQuantityInWEI) {
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] insufficient balance: ${currentSellTokenBalanceInWEI} < ${requiredSellQuantityInWEI}`);
                                await callback({
                            ...
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] sellAction response: ${JSON.stringify(sellResponse)}`);

                            // process sell response
                            if (sellResponse.success == false) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] sellAction failed: ${JSON.stringify(sellResponse)}`);
                                await callback?.({
                            ...
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] buyAction response: ${JSON.stringify(buyResponse)}`);

                            if (buyResponse.success == false) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [${swapType}] [BUY_QUANTITY] buyAction failed: ${JSON.stringify(buyResponse)}`);
                                await callback?.({
                                    text: `\nAn error occurred while performing the swap operation. Please try again.`,
                                    content: {
                                        error: "SWAP_OPERATION_FAILED",
                            ...
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BUY_QUANTITY] buyAction response: ${JSON.stringify(buyResponse)}`)
                            ...
                            if (value_type && value_type == "USD") {  // if value type is USD then convert to USDC
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [VALUE_TYPE]: ${value_type}`);
                                try {
                                    // const price = await get0xPrice({
                                    //     moxieUserId,
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] price: ${price}`);
                                    moxieInWEI = BigInt(price);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] moxieInWEI: ${moxieInWEI}`);

                                    // use the moxie to get the sell quantity in WEI
                                    const currentPriceInWEIMoxie = Decimal(sellTokenSubjectTokenDetails.currentPriceInWeiInMoxie);
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] result: ${result}`);

                                    quantityInWEI = ethers.parseEther(result);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] quantityInWEI: ${quantityInWEI}`);

                                    // execute sell action
                                    const swapResp = await executeSellAction(traceId, moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] swap response: ${JSON.stringify(swapResp)}`);

                                    if (swapResp.success == false) {
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                        return true;
                                    }
                                    ...
                                    // execute buy action
                                    const buyResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, moxieInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] buyResp: ${JSON.stringify(buyResp)}`);
                                } catch (error) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [USD_VALUE_TYPE] [ERROR] Error: ${error}`);
                                    return true;
                                }
                            } else { // this is for non USD value type
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] sellQuantityInWEI: ${sellQuantityInWEI}`);

                                quantityInWEI = sellQuantityInWEI;
                                // execute sell action
                                const swapResp = await executeSellAction(traceId, moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] [MOXIE] executeSellAction  response: ${JSON.stringify(swapResp)}`);

                                if (swapResp.success == false) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] [MOXIE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                    return true;
                                }
                                moxieInWEI = 'moxieReceived' in swapResp
                                    ? ethers.parseUnits(swapResp.moxieReceived, MOXIE_TOKEN_DECIMALS)
                                ...
                                // execute buy action
                                const buyResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, moxieInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [DEFAULT] [MOXIE] buyResp: ${JSON.stringify(buyResp)}`);
                            }
                        } else if (balance && balance.type) { // balance based swap
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] balance type: ${JSON.stringify(balance)}`);
                            try {
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] quantityInWEI: ${quantityInWEI}`);
                                // execute sell action
                                const swapResp = await executeSellAction(traceId,moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] [MOXIE] executeSellAction  response: ${JSON.stringify(swapResp)}`);

                                if (swapResp.success == false) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] [MOXIE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                    return true;
                                }
                                moxieInWEI = 'moxieReceived' in swapResp
                                    ? ethers.parseUnits(swapResp.moxieReceived, MOXIE_TOKEN_DECIMALS)
                                ...
                                // execute buy action
                                    const buyResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, moxieInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] [MOXIE] buyResp: ${JSON.stringify(buyResp)}`);

                                if (buyResp.success == false) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] [MOXIE] buyAction failed: ${JSON.stringify(buyResp)}`);
                                    return true;
                                }

                            } catch (error) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_CREATOR] [BALANCE_BASED] Error getting balance based quantity: ${error}`);
                                return true;
                            }
                        }

                    } else if (swapType == 'CREATOR_TO_TOKEN') {
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] swap type: ${swapType}`);
                        ...
                        let quantityInWEI: bigint;
                        if (buyQuantity) {
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BUY_QUANTITY] quantityInWEI: ${quantityInWEI}`);
                            if (value_type && value_type == "USD") {
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE]: ${value_type}`);
                                try {
                                    // const price = await get0xPrice({
                                    //     moxieUserId,
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] price: ${price}`);
                                    quantityInWEI = BigInt(price);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] quantityInWEI from getUSDEquivalentPrice: ${quantityInWEI}`);
                                } catch (error) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE]: ${value_type} Error getting price: ${error}`);
                                    return true;
                                }
                            } else {
                                if (buyTokenSymbol === "MOXIE") {
                                    ...
                                    try {
                                        ...
                                        quantityInWEI = BigInt(price);
                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] quantityInWEI from getPrice: ${quantityInWEI}`);
                                    } catch (error) {
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] Error getting price: ${error}`);
                                        return true;
                                    }
                                }
                            }
                             ...
                             elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] requiredSellQuantity: ${requiredSellQuantity}`);
                             const requiredSellQuantityInWEI = ethers.parseUnits(requiredSellQuantity, sellTokenDecimals);
                             elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] requiredSellQuantityInWEI: ${requiredSellQuantityInWEI}`);
                             ...
                             elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] availableTokenBalanceInWEI: ${availableTokenBalanceInWEI}`);

                             if (BigInt(availableTokenBalanceInWEI) < BigInt(requiredSellQuantityInWEI)) {
                                 elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] insufficient balance: ${availableTokenBalanceInWEI} < ${requiredSellQuantityInWEI}`);
                                 await callback({
                            ...
                            const swapResp = await executeSellAction(traceId, moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] swap response: ${JSON.stringify(swapResp)}`);

                            if (swapResp.success == false) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                return true;
                            }
                            ...
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] moxieInWEI from executeSellAction: ${moxieInWEI}`);
                            ...
                            if (buyTokenSymbol !== "MOXIE") {
                                ...
                                try {
                                    ...
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [VALUE_TYPE] Error swapping to MOXIE: ${error}`);
                                    return true;
                                }
                            }
                        } else if (sellQuantity) {
                            ...
                            if (value_type && value_type == "USD") {  // if value type is USD then convert to USDC
                                ...
                                try {
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] price: ${price}`);
                                    moxieInWEI = BigInt(price);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] moxieInWEI from getPrice: ${moxieInWEI}`);

                                    // use the moxie to get the sell quantity in WEI
                                    const currentPriceInWEIMoxie = Decimal(sellTokenSubjectTokenDetails.currentPriceInWeiInMoxie);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] currentPriceInWEIMoxie: ${currentPriceInWEIMoxie}`);
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] result: ${result}`);

                                    quantityInWEI = ethers.parseEther(result);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] quantityInWEI: ${quantityInWEI}`);

                                    // execute sell action
                                    const swapResp = await executeSellAction(traceId, moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] swap response: ${JSON.stringify(swapResp)}`);

                                    if (swapResp.success == false) {
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                        return true;
                                    }
                                    moxieInWEI = 'moxieReceived' in swapResp
                                        ? ethers.parseUnits(swapResp.moxieReceived, MOXIE_TOKEN_DECIMALS)
                                        : 0n;
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] moxieInWEI from executeSellAction: ${moxieInWEI}`);
                                    if (buyTokenSymbol !== "MOXIE") {
                                        // swap to the requested buy token
                                        try {
                                            const buyAmountInWEI = await swap(
                                            ...
                                            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] Error swapping to MOXIE: ${error}`);
                                            return true;
                                        }
                                    }
                                } catch (error) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [USD_VALUE_TYPE] Error getting USD value quantity: ${error}`);
                                    return true;
                                }
                            } else {
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] sellQuantityInWEI: ${sellQuantityInWEI}`);
                                ...
                                if (buyTokenSymbol === "MOXIE") {
                                    ...
                                    // execute sell action
                                    const swapResp = await executeSellAction(traceId, moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] executeSellAction response: ${JSON.stringify(swapResp)}`);

                                    if (swapResp.success == false) {
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                        return true;
                                    }
                                    ...
                                    try {
                                        ...
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] Error swapping to MOXIE: ${error}`);
                                        return true;
                                    }
                                }
                            }
                        } else if (balance && balance.type) { // balance based swap
                            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] balance type: ${JSON.stringify(balance)}`);
                            try {
                                const result = await getTargetQuantityForBalanceBasedSwaps(
                                    traceId,
                                    currentWalletBalanceForBalanceBasedSwaps[sellTokenSubjectTokenAddress],
                                ...
                                quantityInWEI = result.quantityInWEI;
                                currentWalletBalanceForBalanceBasedSwaps[sellTokenSubjectTokenAddress] = result.currentWalletBalance;
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] quantityInWEI: ${quantityInWEI}`);

                                // execute sell action
                                const swapResp = await executeSellAction(traceId, moxieUserId, provider, agentWallet.address, sellTokenSubjectAddress, sellTokenSubjectTokenAddress, quantityInWEI, callback, walletClient);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] [MOXIE] executeSellAction  response: ${JSON.stringify(swapResp)}`);

                                if (swapResp.success == false) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] [MOXIE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                    return true;
                                }
                                moxieInWEI = 'moxieReceived' in swapResp
                                    ? ethers.parseUnits(swapResp.moxieReceived, MOXIE_TOKEN_DECIMALS)
                                    : 0n;
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] [MOXIE] moxieInWEI: ${moxieInWEI}`);
                                ...
                                if (buyTokenSymbol !== "MOXIE") {
                                    ...
                                    try {
                                        ...
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] [MOXIE] Error swapping to MOXIE: ${error}`);
                                        return true;
                                    }
                                }
                            } catch (error) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] Error getting balance based quantity: ${error}`);
                                return true;
                            }
                        }
                    } else if (swapType == 'TOKEN_TO_CREATOR') {
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] swap type: ${swapType}`);
                        ...
                        if (buyQuantity) {
                            ...
                            if (value_type && value_type == "USD") {  // this case is for the case where the user is asking to purchase in terms of buy quantity in USD
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [VALUE_TYPE]: ${value_type}`);
                                try {
                                    if (sellTokenSymbol != "USDC") { // dont need to get price if selling USDC
                                        ...
                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [USD_VALUE_TYPE] price from getPrice: ${price}`);
                                        buyQuantityInWEI = BigInt(price);

                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [USD_VALUE_TYPE] buyQuantityInWEI: ${buyQuantityInWEI}`);
                                    ...
                                    if (sellTokenSymbol != "MOXIE") {
                                        try {
                                            ...
                                            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [USD_VALUE_TYPE] Error swapping to MOXIE: ${error}`);
                                            return true;
                                        }
                                    }

                                    // check if user has enough moxie to complete this purchage
                                    const moxieBalance = await getERC20Balance(traceId, MOXIE_TOKEN_ADDRESS, agentWallet.address);
                                    const currentBalance = moxieBalance !== "" ? BigInt(moxieBalance) : 0n;
                                    ...
                                    // execute buy action
                                    const swapResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, buyQuantityInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [USD_VALUE_TYPE] swap response: ${JSON.stringify(swapResp)}`);

                                    if (swapResp.success == false) {
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [USD_VALUE_TYPE] executeSellAction failed: ${JSON.stringify(swapResp)}`);
                                        return true;
                                    }
                                } catch (error) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] [USD_VALUE_TYPE] Error getting price: ${error}`);
                                    return true;
                                }
                                ...
                                moxieInWEI = await calculateTokensBuy(traceId, moxieUserId, buyTokenSubjectAddress, buyQuantityInWEI);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] moxie in WEI: ${moxieInWEI}`);
                                ...
                                if (sellTokenSymbol == "MOXIE") {
                                    ...
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] price from getPrice: ${price}`);
                                    moxieInWEI = BigInt(price);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] moxie in WEI: ${moxieInWEI}`);
                                    ...
                                    try {
                                        ...
                                        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] Error swapping to MOXIE: ${error}`);
                                        return true;
                                    }
                                }
                                ...
                                // execute buy action
                                const swapResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, quantityInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] swap response: ${JSON.stringify(swapResp)}`);

                                if (swapResp.success == false) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [BUY_QUANTITY] executeBuyAction failed: ${JSON.stringify(swapResp)}`);
                                    return true;
                                }
                            }
                        } else if (sellQuantity) {
                            ...
                            if (value_type && value_type == "USD") {  // if value type is USD then convert to USDC
                                sellQuantityInWEI = ethers.parseUnits(sellQuantity.toString(), USDC_TOKEN_DECIMALS);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [VALUE_TYPE]: ${value_type}`);
                                try {
                                    if (sellTokenSymbol != "USDC") { // dont need to get price if selling USDC
                                        // const price = await get0xPrice({
                                        //     moxieUserId,
                                        ...
                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [USD_VALUE_TYPE] price from getUSDEquivalentPrice: ${price}`);
                                        sellQuantityInWEI = BigInt(price);

                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [USD_VALUE_TYPE] sellQuantityInWEI: ${sellQuantityInWEI}`);
                                    ...
                                    if (sellTokenSymbol != "MOXIE") {
                                        ...
                                        try {
                                            ...
                                            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [USD_VALUE_TYPE] Error swapping to MOXIE: ${error}`);
                                            return true;
                                        }
                                    }

                                    // execute buy action
                                    const swapResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, sellQuantityInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_CREATOR] [USD_VALUE_TYPE] swap response: ${JSON.stringify(swapResp)}`);
                                ...
                                if (sellTokenSymbol === "MOXIE") {
                                    ...
                                    // execute buy action
                                    const swapResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, buyAmountInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [DEFAULT] executeSellAction response: ${JSON.stringify(swapResp)}`);
                            ...
                            try {
                                ...
                                // execute buy action
                                const swapResp = await executeBuyAction(traceId, moxieUserId, provider, agentWallet.address, buyTokenSubjectAddress, quantityInWEI, callback, walletClient, buyTokenSubjectTokenDetails.name);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] executeBuyAction response: ${JSON.stringify(swapResp)}`);

                                if (swapResp.success == false) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] executeBuyAction failed: ${JSON.stringify(swapResp)}`);
                                    return true;
                                }
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] swap response: ${JSON.stringify(swapResp)}`);
                            } catch (error) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CREATOR_TO_TOKEN] [BALANCE_BASED] Error getting balance based quantity: ${error}`);
                                return true;
                            }
                        ...
                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] swap type: ${swapType}`);
                        if (buyQuantity) {
                            let buyQuantityInWEI = ethers.parseUnits(buyQuantity.toString(), buyTokenDecimals);
                            if (value_type && value_type == "USD") {  // this case is for the case where the user is asking to purchase in terms of buy quantity in USD
                                buyQuantityInWEI = ethers.parseUnits(buyQuantity.toString(), USDC_TOKEN_DECIMALS);
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] [VALUE_TYPE]: ${value_type}`);
                                try {
                                    if (sellTokenSymbol != "USDC") { // dont need to get price if selling USDC
                                        // const price = await get0xPrice({
                                        //     moxieUserId,
                                        ...
                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] [USD_VALUE_TYPE] price from getUSDEquivalentPrice: ${price}`);
                                        buyQuantityInWEI = BigInt(price);

                                        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] [USD_VALUE_TYPE] buyQuantityInWEI: ${buyQuantityInWEI}`);
                                    }
                                    ...
                                    if (sellTokenSymbol != buyTokenSymbol) {
                                        try {
                                            ...
                                            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] [USD_VALUE_TYPE] Error swapping to MOXIE: ${error}`);
                                            return true;
                                        }
                                    }

                                } catch (error) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] [USD_VALUE_TYPE] Error getting price: ${error}`);
                                    return true;
                                }
                            } else {
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] price from getUSDEquivalentPrice: ${price}`);
                                ...
                                if (BigInt(currentSellTokenBalanceInWEI) < BigInt(price)) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] insufficient balance: ${currentSellTokenBalanceInWEI} < ${Number(price)}`);
                                ...
                                try {
                                    ...
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BUY_QUANTITY] Error swapping to MOXIE: ${error}`);
                                    return true;
                                }
                            }
                            ...
                            if (value_type && value_type == "USD" && sellTokenAddress !== USDC_ADDRESS) {
                                ...
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [SELL_QUANTITY] [USD_VALUE_TYPE] priceOfSellTokenFromUSDInWei: ${priceOfSellTokenFromUSDInWei}`);
                                ...
                                if (BigInt(currentSellTokenBalanceInWEI) < sellQuantityInWEI) {
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [SELL_QUANTITY] [USD_VALUE_TYPE] insufficient balance: ${currentSellTokenBalanceInWEI} < ${Number(priceOfSellTokenFromUSDInWei)}`);
                                    await callback({
                                    ...
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [SELL_QUANTITY] [USD_VALUE_TYPE] Error swapping to MOXIE: ${error}`);
                                    return true;
                                }
                                ...
                                if (BigInt(currentSellTokenBalanceInWEI) < sellQuantityInWEI) {
                                    ...
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [SELL_QUANTITY] Error swapping to MOXIE: ${error}`);
                                    return true;
                                }
                            }
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BALANCE_BASED] Error swapping to MOXIE: ${error}`);
                                    return true;
                                }
                                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BALANCE_BASED] quantityInWEI: ${quantityInWEI}`);
                            } catch (error) {
                                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [TOKEN_TO_TOKEN] [BALANCE_BASED] Error getting balance based quantity: ${error}`);
                                return true;
                            }
                ...
                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] [CACHE] deleted cache key: ${cacheKey}`);
            }
        } catch (error) {
            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [SWAP] error occured while performing swap operation: ${JSON.stringify(error)}`);
...
async function isValidSwapContent(
    ...
    if (!content || !content.transactions || content.transactions.length === 0) {
        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [isValidSwapContent] Invalid content structure: ${JSON.stringify(content)}`);
        await callback?.({
    ...
    for (const transaction of content.transactions) {
        ...
        if (!transaction.sellToken || !transaction.buyToken || (!transaction.balance && !transaction.buyQuantity && !transaction.sellQuantity)) {
            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [isValidSwapContent] Missing required fields in transaction: ${JSON.stringify(transaction)}`);
            await callback?.({
                text: "\nAn error occurred while processing your request. Please try again.",
                content: {
        ...
        if ((transaction.sellQuantity && transaction.sellQuantity <= 0) ||
            ...
            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [isValidSwapContent] Invalid quantity: sellQuantity=${transaction.sellQuantity}, buyQuantity=${transaction.buyQuantity}`);
            await callback?.({
        ...
        if (transaction.balance && transaction.balance.type) {
            if (!transaction.balance.source_token ||
                ...
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [isValidSwapContent] Invalid balance configuration: ${JSON.stringify(transaction.balance)}`);
                await callback?.({
                    text: "\nAn error occurred while processing your request. Please try again.",
                    content: {
...
async function swap(
    ...
    try {
        ...
        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] balance: ${balance}`);
        ...
        if (tokenBalance < sellAmountInWEI) {
            ...
            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] Insufficient balance for ${sellTokenSymbol} to ${buyTokenSymbol} swap. Token balance: ${tokenBalance}, required: ${sellAmountInWEI}`);
            throw new Error(`[tokenSwap] [${moxieUserId}] [swap] Insufficient balance for ${sellTokenSymbol} to ${buyTokenSymbol} swap. Token balance: ${tokenBalance}, required: ${sellAmountInWEI}`);
        }
        ...
        if (!quote.liquidityAvailable) {
            elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [swap] liquidity not available for ${sellTokenSymbol} to ${buyTokenSymbol} swap`);
            await callback?.({
                    text: `\nInsufficient liquidity to complete this transaction. Please try with a smaller amount.`,
            });
            throw new Error(`[tokenSwap] [${moxieUserId}] [swap] Insufficient liquidity for ${sellTokenSymbol} to ${buyTokenSymbol} swap`);
        }
        ...
        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] issues from get0xSwapQuote: ${JSON.stringify(issues)}`);
        // check allowance and approve spending
        if (issues.allowance && issues.allowance != null) {
            await checkAllowanceAndApproveSpendRequest(
                ...
                walletClient,
                callback);
            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] checkAllowanceAndApproveSpendRequest completed`);
        }
        ...
        if (issues.balance && issues.balance != null) {
            const balance = sellTokenSymbol === "ETH"
                ? await provider.getBalance(agentWalletAddress)
                : await getERC20Balance(traceId, sellTokenAddress, agentWalletAddress);
            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] tokenBalance: ${balance}`);
            if (balance) {
        ...
        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [swap] Error getting 0x quote: ${error.message}`);
        ...
        throw new Error(`[tokenSwap] [${moxieUserId}] [swap] Error getting 0x quote: ${error.message}`);
    }
    ...
    try {
        ...
        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] 0x tx: ${JSON.stringify(tx)}`);
    } catch (error) {
        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [swap] Error executing 0x swap: ${JSON.stringify(error)}`);
        await callback?.({
            text: `\nAn error occurred while processing your request. Please try again.`,
            content: {
        ...
        throw new Error(`[tokenSwap] [${moxieUserId}] [swap] Error executing 0x swap: ${JSON.stringify(error)}`);
    }
    ...
    try {
        txnReceipt = await handleTransactionStatus(traceId, moxieUserId, provider, tx.hash);
        if (!txnReceipt) {
    ...
    if (txnReceipt.status == 1) {
        ...
        if (buyTokenAddress !== ETH_ADDRESS) {
            ...
            if (transferDetails) {
                ...
                elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [swap] buyAmountInWEI: ${buyAmountInWEI}`)
            } else {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [swap] Error decoding token transfer`);
                await callback?.({
                    text: `\nAn error occurred while processing your request. Please try again.`,
                    content: {
...
async function getTargetQuantityForBalanceBasedSwaps(
    ...
    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [getTargetQuantityForBalanceBasedSwaps] currentWalletBalance: ${currentWalletBalance} ${sellTokenAddress}`);
    if (!currentWalletBalance || currentWalletBalance === 0n) {
        elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [balance] currentWalletBalance is ${currentWalletBalance}`);
        await callback?.({
            text: `\nYour agent wallet doesn't have any ${sellTokenSymbol} balance to complete this operation.`,
        });
        throw new Error(`[tokenSwap] [${moxieUserId}] [tokenSwapAction] [balance] currentWalletBalance is ${currentWalletBalance}`);
    }
...
async function getFtaResponses(
    ...
    for (const creatorId of creatorIds) {
        ...
        if (ftaResponse) {
            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] fta response fetched successfully from cache for creator moxie user id: ${creatorId}, ${JSON.stringify(ftaResponse)}`);
            ...
            if (!newFtaResponse || newFtaResponse == null) {
                elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] fta response not found for creator ${creatorId}`);
                await callback?.({
                ...
                throw new Error(`[tokenSwap] [${moxieUserId}] [tokenSwapAction] The creator with ID ${creatorId} could not be found. Please verify the creator ID`);
            }
            await runtime.cacheManager.set(`userftadetails-${creatorId}`, newFtaResponse);
            ftaResponses[creatorId] = newFtaResponse;
            elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [tokenSwapAction] fta response fetched successfully for creator ${creatorId} and set in cache`);
        }
    }
    return ftaResponses;
}
...
async function getCreatorCoinDetails(
    ...
    if (creatorIds.length > 0) {
        ...
        try {
            ftaResponses = await getFtaResponses(traceId, creatorIds, moxieUserId, runtime, callback);
        } catch (error) {
    ...
    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [getCreatorCoinDetails]  [SELL_TOKEN_DETAILS]: ${JSON.stringify(sellTokenSubjectTokenDetails)}`);
    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [getCreatorCoinDetails]  [BUY_TOKEN_DETAILS]: ${JSON.stringify(buyTokenSubjectTokenDetails)}`);
...
async function handleInsufficientBalance(
    ...
    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [handleInsufficientBalance] [otherTokenSymbols]: ${JSON.stringify(otherTokenSymbols)}`);
    ...
    elizaLogger.debug(traceId,`[tokenSwap] [${moxieUserId}] [handleInsufficientBalance] [otherTokenSymbolsMap]: ${JSON.stringify(otherTokenSymbolsMap)}`);
