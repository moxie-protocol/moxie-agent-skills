...
export async function createGroup(
    ...
    try {
        ...
        elizaLogger.debug("createGroup fetch completed", {
            status: data.status,
        });
        ...
        if (result.errors) {
            ...
            throw new Error(
                `Failed to create group: ${result.errors[0].message}`
            );
        }

        elizaLogger.info("createGroup successful", {
...
export async function updateGroup(
    ...
    try {
        ...
        elizaLogger.debug("updateGroup fetch completed", {
            status: data.status,
        });
        ...
        if (result.errors) {
            ...
            throw new Error(
                `Failed to update group: ${result.errors[0].message}`
            );
        }

        elizaLogger.info("updateGroup successful", {
            data: result.data.UpdateGroup,
        });
...
export async function deleteGroup(
    ...
    try {
        ...
        elizaLogger.debug("deleteGroup fetch completed", {
            status: data.status,
        });
        ...
        if (result.errors) {
            ...
            throw new Error(
                `Failed to delete group: ${result.errors[0].message}`
            );
        }

        elizaLogger.info("deleteGroup successful", {
            data: result.data.DeleteGroup,
        });
...
export async function addMembersToGroup(
    ...
    try {
        ...
        elizaLogger.debug("addMembersToGroup fetch completed", {
            status: data.status,
        });

        const result = await data.json();
        if (result.errors) {
            elizaLogger.error("addMembersToGroup failed", {
                errors: result.errors,
            });
            throw new Error(
                `Failed to add members to group: ${result.errors[0].message}`
            );
        }

        elizaLogger.info("addMembersToGroup successful", {
            data: result.data.AddMembersToGroup,
        });
        return result.data.AddMembersToGroup;
    } catch (error) {
        elizaLogger.error("Error in addMembersToGroup", {
            error: error.message,
        });
...
export async function removeMembersFromGroup(
    ...
    try {
        const input: ModifyGroupMembersInput = { groupId, members };
        elizaLogger.debug("removeMembersFromGroup input constructed", {
            input,
        });
        ...
        elizaLogger.debug("removeMembersFromGroup fetch completed", {
            status: data.status,
        });

        const result = await data.json();
        if (result.errors) {
            elizaLogger.error("removeMembersFromGroup failed", {
                errors: result.errors,
            });
            throw new Error(
                `Failed to remove members from group: ${result.errors[0].message}`
            );
        }

        elizaLogger.info("removeMembersFromGroup successful", {
            data: result.data.RemoveMembersFromGroup,
        });
        return result.data.RemoveMembersFromGroup;
    } catch (error) {
        elizaLogger.error("Error in removeMembersFromGroup", {
            error: error.message,
        });
...
const errorMessages: Record<string, string> = {
    ...
    AERR102:
        "Group names must not begin with special characters such as #, $, or @.",
    AERR103:
    ...
    AERR105:
        "The user does not have ownership rights to update the group details.",
    ...
    AERR110:
        "The provided Senpi user IDs are invalid for adding members to the group.",
    AERR111: "You cannot add yourself to the group.",
    AERR112:
        "The user does not have ownership rights to add members to the group.",
    ...
    AERR115:
        "The user does not have ownership rights to remove members from the group.",
