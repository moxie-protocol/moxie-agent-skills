...
import { AppBalance, AppTokenPosition, Portfolio, TokenBalance } from "../../types";
...
async function generatePortfolioSummary(
    ...
    console.log("portfolioDataFiltered--generatePortfolioSummary-->",portfolioDataFiltered);
...
async function getWalletAddresses(userInfo: MoxieUser) {
    const addresses = userInfo.wallets.map(wallet => wallet?.walletAddress) || [];
    const vestingAddresses = userInfo.vestingContracts?.map(contract => contract?.vestingContractAddress) || [];
...
async function handleMultipleUsers(
    ...
    for (const userInfo of moxieUserInfoMultiple) {
        ...
        const portfolioData = await getPortfolioData(walletAddresses, ["BASE_MAINNET"], userInfo.id, runtime);
        if (!portfolioData || (portfolioData.tokenBalances.length === 0 && portfolioData.appBalances.length === 0)) {
            continue;
        }
        const tokenBalancesFiltered = portfolioData.tokenBalances.filter(token => token.token.balanceUSD > 0.01)
        .sort((a, b) => b.token.balanceUSD - a.token.balanceUSD)
        .slice(0, 20);

        const appBalancesFiltered = portfolioData.appBalances
        .filter(app => app.products.some(prod =>
            prod.assets.some(asset => asset.balanceUSD > 0.01)
        ))
        .sort((a, b) => {
            const aMaxBalance = Math.max(...a.products.flatMap(p =>
                p.assets.map(asset => asset.balanceUSD || 0)
            ));
            const bMaxBalance = Math.max(...b.products.flatMap(p =>
                p.assets.map(asset => asset.balanceUSD || 0)
            ));
...
export default {
    ...
    description: "Get portfolio summary showing token holdings with amounts, USD values and percentages, plus creator coin holdings if present. Includes total portfolio value and relevant insights. It also compute portfolio for multiple users if requested",
    handler: async (
        runtime: IAgentRuntime,
        message: Memory,
        state: State,
        ...
        try {
            ...
            if (requestedMoxieUserIds?.length > 0) {
                moxieUserInfoMultiple = await Promise.all(
                    requestedMoxieUserIds.map(id => moxieUserService.getUserByMoxieId(id))
                );
                moxieUserInfo = moxieUserInfoMultiple[0]; // Keep first one for backwards compatibility

                if (!moxieUserInfo || moxieUserInfoMultiple.some(user => !user)) {
                    await callback({
                ...
                if (moxieUserInfoMultiple.length > 1) {
                    const portfolioSummaries = await handleMultipleUsers(moxieUserInfoMultiple, message, runtime);
                ...
                const moxieUserInfoRaw = await getMoxieCache(userPortfolioCacheKey, runtime);
                if(moxieUserInfoRaw) {
                    const summary = await handleCachedPortfolio(moxieUserInfoRaw, message, runtime);
            ...
            const summary = await generatePortfolioSummary(portfolioData, message, runtime);

            elizaLogger.success("[Portfolio] Successfully generated portfolio summary");
            setMoxieCache(summary, cacheResponeKey, runtime);
            setMoxieCache(JSON.stringify(moxieUserInfo), userPortfolioCacheKey, runtime);
