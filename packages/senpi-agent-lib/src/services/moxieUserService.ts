    ...
    try {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (
                    !data?.GetUserInfoBatch?.users ||
                    data.GetUserInfoBatch.users.length === 0
                ) {
                    retryCount++;
                    elizaLogger.warn(
                        `Retry ${retryCount}: Empty users array received`
                    );
        ...
        if (retryCount >= maxRetries) {
            elizaLogger.error(
                "Failed to get user info batch after maximum retries"
            );
            throw new Error(
                "Failed to get user info batch after maximum retries"
            );
