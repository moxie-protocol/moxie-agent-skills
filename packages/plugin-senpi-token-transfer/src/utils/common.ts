...
} from "../templates/callBackTemplate";
...
export async function handleTransactionStatus(
    ...
    while (retryCount < MAX_RETRIES) {
        try {
            elizaLogger.debug(
                `[${context.senpiUserId}] [handleTransactionStatus] Attempt ${retryCount + 1} of ${MAX_RETRIES}`
            );
            ...
            if (!txnReceipt) {
                elizaLogger.warn(
                    `[${context.senpiUserId}] [handleTransactionStatus] Transaction receipt timeout on attempt ${retryCount + 1}`
                );
                retryCount++;
                ...
                if (retryCount < MAX_RETRIES) {
                    ...
                    elizaLogger.debug(
                        `[${context.senpiUserId}] [handleTransactionStatus] Retrying in ${delay}ms...`
                    );
                    await new Promise((resolve) => setTimeout(resolve, delay));
                    continue;
                } else {
                    elizaLogger.error(
                        `[${context.senpiUserId}] [handleTransactionStatus] Transaction receipt timeout after ${MAX_RETRIES} attempts`
                    );
                    return {
                        data: null,
                        callBackTemplate:
                            TRANSACTION_VERIFICATION_TIMEOUT(txHash),
                    };
                }
            }
            ...
            if (txnReceipt.status === 1) {
                ...
                return {
                    data: null,
                    callBackTemplate: TRANSACTION_FAILED(
                        txHash,
                        "Transaction failed"
                    ),
                };
            ...
            elizaLogger.warn(
                `[${context.senpiUserId}] [handleTransactionStatus] Error on attempt ${retryCount}: ${errorMessage}`
            );
    ...
    elizaLogger.error(
        `[${context.senpiUserId}] [handleTransactionStatus] Failed after ${MAX_RETRIES} attempts: ${errorMessage}`
    );
    return {
