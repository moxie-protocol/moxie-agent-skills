 ...
 */
export async function decodeTokenTransfer(moxieUserId: string, txReceipt: ethers.providers.TransactionReceipt, buyTokenAddress: string, agentWalletAddress: string): Promise<{ amount: string, from: string, to: string } | null> {
    try {
        ...
        if (buyTokenAddress !== ETH_ADDRESS && buyTokenAddress !== WETH_ADDRESS) {
            elizaLogger.debug(`[decodeTokenTransfer] [${moxieUserId}] fetching token log for transaction for ERC20: ${txReceipt.transactionHash}`);
            ...
            if (!tokenLog) {
                elizaLogger.error(`[decodeTokenTransfer] [${moxieUserId}] No token transfer event found in transaction receipt`);
                return null
            }
        ...
        elizaLogger.error(`[decodeTokenTransfer] [${moxieUserId}] Error decoding token transfer: ${JSON.stringify(error)}`);
        return null
    }
}
...
export async function getERC20Decimals(context: Context, tokenAddress: string) {
    ...
    try {
        ...
        if (cachedDecimals !== null && cachedDecimals !== undefined) {
            elizaLogger.debug(context.traceId, `[${context.moxieUserId}] [getERC20Decimals] Found decimals in cache for token ${tokenAddress}: ${cachedDecimals}`);
        ...
        elizaLogger.error(context.traceId, `[${context.moxieUserId}] [getERC20Decimals] Error fetching token decimals: ${JSON.stringify(error)}`);
        throw error;
    }
}