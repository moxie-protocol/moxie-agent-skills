...
export async function getPrice(
    ...
    try {
        ...
        [sourceTokenPriceInUSD, targetTokenPriceInUSD] = await Promise.all([
            fetchPriceWithRetry(sourceTokenAddress, sourceTokenSymbol, traceId, moxieUserId),
            fetchPriceWithRetry(targetTokenAddress, targetTokenSymbol, traceId, moxieUserId)
        ]);
...
export async function fetchPriceWithRetry(tokenAddress: string, tokenSymbol: string, traceId: string, moxieUserId: string): Promise<number> {
    let lastError;
