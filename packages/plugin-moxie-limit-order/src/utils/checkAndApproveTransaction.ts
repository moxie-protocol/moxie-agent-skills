...
import { approvalTransactionSubmitted, approvalTransactionConfirmed, approvalTransactionFailed, approvalTransactionTimedOut } from './callbackTemplates';
...
export async function checkAllowanceAndApproveSpendRequest(
    ...
    elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] called, walletAddress: ${walletAddress}, tokenAddress: ${tokenAddress}, spenderAddress: ${spenderAddress}, tokenAmount: ${amountInWEI}`)
    try {
        ...
        elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] current ${spenderAddress} allowance for wallet address: ${walletAddress}, ${currentAllowance}`)

        // If allowance is already sufficient, return early
        if (currentAllowance && currentAllowance >= amountInWEI) {
            elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] Sufficient allowance already exists. hence no approval is required`);
            return true;
        }

        elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] Sufficient allowance not exists. hence proceeeding with approval is required`);
        ...
        elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] feeData: ${JSON.stringify(feeData)}`)
        ...
        while (retryCount < MAX_RETRIES) {
            try {
                ...
                retryCount++;
                elizaLogger.warn(traceId, `[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] Transaction attempt ${retryCount} failed: ${error.message}`);
                if (retryCount >= MAX_RETRIES) {
                    elizaLogger.error(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] [ERROR] Failed to send transaction after ${MAX_RETRIES} attempts`);
                    await callback(approvalTransactionFailed(approveResponse.hash));
        ...
        if (approveResponse && approvalTxHash) {
            ...
            while (receiptRetryCount < MAX_RECEIPT_RETRIES) {
                try {
                    ...
                    receiptRetryCount++;
                    elizaLogger.warn(traceId, `[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] Transaction receipt attempt ${receiptRetryCount} failed: ${error.message}`);
                    if (receiptRetryCount >= MAX_RECEIPT_RETRIES) {
                        elizaLogger.error(traceId, `[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] [ERROR] Failed to get transaction receipt after ${MAX_RECEIPT_RETRIES} attempts`);
                        await callback(approvalTransactionTimedOut(approvalTxHash));
                        throw error; // Rethrow the error after all retries are exhausted
                    }
                    // Wait before retrying (exponential backoff)
                    await new Promise(resolve => setTimeout(resolve, 1000 * Math.pow(2, receiptRetryCount)));
                }
            }
            elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] approval tx receipt: ${JSON.stringify(receipt)}`)
            if (receipt.status === 1) {
                elizaLogger.debug(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] [SUCCESS] Approval transaction successful: ${approvalTxHash}`);
                await callback(approvalTransactionConfirmed(approvalTxHash));
            } else {
                elizaLogger.error(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] [ERROR] Approval transaction failed: ${approvalTxHash}`);
                await callback(approvalTransactionFailed(approvalTxHash));
                throw new Error(`Approval transaction failed`);
            }
        } else {
            elizaLogger.error(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] [ERROR] No transaction hash returned for approval`);
            throw new Error(`Approval transaction not initiated`);
        }
        return true;
    } catch (error) {
        elizaLogger.error(traceId,`[${moxieUserId}] [checkAllowanceAndApproveSpendRequest] [ERROR] error in checkAllowanceAndApproveSpendRequest, ${JSON.stringify(error)}`)
        throw error
    }
