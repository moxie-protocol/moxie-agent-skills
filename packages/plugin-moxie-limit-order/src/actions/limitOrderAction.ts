...
import { generateObjectDeprecated, composeContext, IAgentRuntime, ModelClass, ModelProviderName, stringToUuid, Content } from "@moxie-protocol/core";
    ...
    handler: async (
        ...
        try {
            ...
            if (limitOrderOptions.callBackTemplate) {
                elizaLogger.debug(traceId, `[limitOrderAction] [${moxieUserId}] [processMessage] limitOrderOptions: ${JSON.stringify(limitOrderOptions)}`);
            ...
            elizaLogger.error(traceId, `[limitOrderAction] [${moxieUserId}] [ERROR] error: ${error}`);
...
async function processMessage(
    ...
    try {
        ...
        elizaLogger.debug(context.traceId, `[limitOrder] [${context.moxieUserId}] limitOrderOptions: ${JSON.stringify(limitOrderOptions)}`);
...
async function processLimitOrder(context: Context, data: LimitOrderResponse) {
    elizaLogger.debug(context.traceId, `[limitOrder] [${context.moxieUserId}] [processLimitOrder] started`);
    ...
    for (const limitOrder of data.limit_orders) {
        elizaLogger.debug(context.traceId, `[limitOrder] [${context.moxieUserId}] [processLimitOrder] Processing limit order: ${JSON.stringify(limitOrder)}`);
        ...
        try {
            ...
            if (limitOrder.execution_type === 'FUTURE') {
                elizaLogger.debug(context.traceId, `[limitOrder] [${context.moxieUserId}] [processLimitOrder] Execution type is future, processing limit order then executing`);
...
async function processSingleLimitOrder(
    ...
    try {
        ...
        elizaLogger.debug(context.traceId, `[limitOrder] [${context.moxieUserId}] [processSingleLimitOrder] sellTokenBalance: ${sellTokenBalance}`);
        ...
        elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [USD_VALUE_TYPE] sellTokenPriceInUSD: ${sellTokenPriceInUSD} and buyTokenPriceInUSD: ${buyTokenPriceInUSD}`);
         ...
         if (limitPrice.type === "PERCENTAGE") {
             ...
             elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [PERCENTAGE] targetPriceInUSD: ${targetTokenPriceInUSD}`);
             ...
             elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [TOKEN_PRICE] targetPriceInUSD: ${targetTokenPriceInUSD}`);
         }
        ...
        if (buyQuantity) {
            try {
                ...
                elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] Error calculating amounts: ${error}`);
                throw error;
            }
            ...
            try {
                ...
                elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] Error calculating amounts: ${error}`);
                throw error;
            }
            ...
            try {
                ...
                elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BALANCE_BASED] Error getting balance based quantity: ${error}`);
                throw error;
            }
        }
        ...
        if (sellTokenSymbol === "ETH") {
            try {
                ...
                elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [ETH_TO_WETH_SWAP] Error swapping ETH to WETH: ${error}`);
                throw error;
            }
        }
        ...
        elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [orderCreation] orderCreation: ${JSON.stringify(orderCreation)}`);
        ...
        elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [checkUserCommunicationPreferences] communicationPreference: ${communicationPreference}`);
        ...
        elizaLogger.error(context.traceId, `[limitOrder] [${context.moxieUserId}] [processSingleLimitOrder] Error: ${error}`);
...
async function checkUserCommunicationPreferences(traceId: string, moxieUserId: string): Promise<string | null> {
    try {
        ...
        if (!response.ok) {
            elizaLogger.error(traceId, `[limitOrder] [${moxieUserId}] Failed to fetch user preferences: ${response.statusText}`);
            return null;
        }

        const data = await response.json();
        elizaLogger.debug(traceId, `[limitOrder] [${moxieUserId}] User communication preferences:`, data?.data?.GetUser?.communicationPreference);
        return data?.data?.GetUser?.communicationPreference;

    } catch (error) {
        elizaLogger.error(traceId, `[limitOrder] [${moxieUserId}] Error checking user preferences: ${error.message}`);
        return null;
    }
}
...
async function getTargetQuantityForBalanceBasedSwaps(
    ...
    elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [getTargetQuantityForBalanceBasedSwaps] currentWalletBalance: ${currentWalletBalance} ${sellTokenAddress}`);
    if (!currentWalletBalance || currentWalletBalance === 0n) {
        elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [balance] currentWalletBalance is ${currentWalletBalance}`);
    ...
    if (sellTokenSymbol === "ETH" && percentage === 100) {
        percentage = 99;
        elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [balance] Using 99% instead of 100% for ETH`);
    }
    
    // Scale up by a larger factor (e.g., 1e7)
    quantityInWEI = (BigInt(currentWalletBalance) * BigInt(percentage * 1e7)) / BigInt(1e9)
    elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [balance] quantityInWEI: ${quantityInWEI}`);
...
async function swap(
    ...
    try {
        ...
        elizaLogger.error(traceId,`[tokenSwap] [${moxieUserId}] [swap] Error signing typed data: ${error}`);
        throw error;
    }
    // }
    ...
    try {
        ...
        elizaLogger.error(traceId,'[tokenSwap] [${moxieUserId}] [swap] Error executing 0x swap:', {error});
        throw error;
    }
    ...
    if (txnReceipt && txnReceipt.status == 1) {
        ...
        if (buyTokenAddress !== ETH_ADDRESS && buyTokenAddress !== WETH_ADDRESS) {
            ...
            const transferDetails = await decodeTokenTransfer( moxieUserId, txnReceipt, buyTokenAddress, agentWalletAddress);
...
const calculateBuyQuantityAmounts = async (
    ...
    if (value_type && value_type == "USD") {
        ...
        try {
            ...
            // Log results
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [USD_VALUE_TYPE] buyAmount: ${buyAmount}, buyAmountInWEI: ${buyTokenAmountInWEI}`);
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [USD_VALUE_TYPE] sellAmount: ${sellAmount}, sellAmountInWEI: ${sellTokenAmountInWEI}`);

            return {buyTokenAmountInWEI, sellTokenAmountInWEI};
        } catch (error) {
            elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [USD_VALUE_TYPE] Error getting price: ${error}`);
            throw error;
        }
    } else {
        ...
        try {
            ...
            // Log results
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [DIRECT_VALUE_TYPE] buyAmount: ${buyAmount}, buyAmountInWEI: ${buyTokenAmountInWEI}`);
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [DIRECT_VALUE_TYPE] sellAmount: ${sellAmount}, sellAmountInWEI: ${sellTokenAmountInWEI}`);

            return {buyTokenAmountInWEI, sellTokenAmountInWEI};
        } catch (error) {
            elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BUY_QUANTITY] [DIRECT_VALUE_TYPE] Error getting price: ${error}`);
            throw error;
        }
    }
};
...
const calculateSellQuantityAmounts = async (
    ...
    if (value_type && value_type == "USD") {
        ...
        try {
            ...
            // Log results
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] [USD_VALUE_TYPE] buyAmount: ${buyAmount}, buyAmountInWEI: ${buyTokenAmountInWEI}`);
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] [USD_VALUE_TYPE] sellAmount: ${sellAmount}, sellAmountInWEI: ${sellTokenAmountInWEI}`);

            return {buyTokenAmountInWEI, sellTokenAmountInWEI};
        } catch (error) {
            elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] [USD_VALUE_TYPE] Error getting price: ${error}`);
            throw error;
        }
    } else {
        ...
        try {
            ...
            // Log results
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] [DIRECT_VALUE_TYPE] buyAmount: ${buyAmount}, buyAmountInWEI: ${buyTokenAmountInWEI}`);
            elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] [DIRECT_VALUE_TYPE] sellAmount: ${sellAmount}, sellAmountInWEI: ${sellTokenAmountInWEI}`);

            return {buyTokenAmountInWEI, sellTokenAmountInWEI};
        } catch (error) {
            elizaLogger.error(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [SELL_QUANTITY] [DIRECT_VALUE_TYPE] Error getting price: ${error}`);
            throw error;
        }
    }
};
...
const calculateBalanceBasedAmounts = async (
    ...
    // Log results
    elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BALANCE_BASED] buyAmount: ${buyAmount}, buyAmountInWEI: ${buyTokenAmountInWEI}`);
    elizaLogger.debug(traceId,`[limitOrder] [${moxieUserId}] [processSingleLimitOrder] [BALANCE_BASED] sellAmount: ${sellAmount}, sellAmountInWEI: ${sellTokenAmountInWEI}`);
